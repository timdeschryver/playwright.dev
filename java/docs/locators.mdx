...
- [Page.getByTitle(text[, options])](/api/class-page.mdx#page-get-by-title) to locate an element by its title attribute.
- [Page.getByTestId(testId)](/api/class-page.mdx#page-get-by-test-id) to locate an element based on its `data-testid` attribute (other attributes can be configured).
