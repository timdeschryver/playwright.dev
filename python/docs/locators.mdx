...
- [page.get_by_title(text, **kwargs)](/api/class-page.mdx#page-get-by-title) to locate an element by its title attribute.
- [page.get_by_test_id(test_id)](/api/class-page.mdx#page-get-by-test-id) to locate an element based on its `data-testid` attribute (other attributes can be configured).
